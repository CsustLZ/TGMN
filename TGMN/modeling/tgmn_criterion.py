...
class TGMNSetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses,
                 num_points, oversample_ratio, importance_sample_ratio, sim_use_clip, loss_gate):
    ...
    def gate_loss(self, outputs, targets, indices, num_masks):
        # compute gumble softmax gate loss which object network shoud propagate or not
        assert 'pred_prop_soft' in outputs
        src_prop_logits = outputs['pred_prop_soft']
        L, B, cQ, _ = src_prop_logits.shape
        src_prop_logits = src_prop_logits.reshape(L * B, cQ, 1)

        idx = self._get_src_permutation_idx(indices)
        valid_targets = []
        for t in (targets * L):
            valid_targets.append(t["valid_inst"])  # valid_inst
        valid_targets = torch.cat(valid_targets)

        target_classes = torch.zeros_like(src_prop_logits, dtype=torch.float, device=src_prop_logits.device, )  # LB, cQ
        valid_idx = (idx[0][valid_targets], idx[1][valid_targets])
        target_classes[valid_idx] = 1

        loss_gate = F.binary_cross_entropy_with_logits(src_prop_logits, target_classes)
        return {'loss_genvis_gate': loss_gate}
